...
@Component({
  selector: 'app-create-public-starup',
  templateUrl: './create-public-starup.component.html',
  styleUrls: ['./create-public-starup.component.scss']
})

export class CreatePublicStarupComponent implements OnInit {
  ...
  ngOnInit(): void {
    setTimeout(()=>{
      this.getEditedAreas()
      this.isDone = this.actualProject.resultado
    }, 3000)
  }
  ...
  saveArea(area: number) {
    ...
    Object.keys(this.ProjectsForm.value.areas[area]).forEach((formKey, i)=>{
      ...
      areasAnswer.push({
        ...
        id_question: parseInt(formKey),
        option_value: this.testAdapter[area].questions[i] ?
        this.testAdapter[area].questions[i].options[res[formKey] - 1]
          ? this.testAdapter[area].questions[i].options[res[formKey] - 1].value
          :
            0
          :
            0,
        result_text:
         this.testAdapter[area].questions[i] ?
            this.testAdapter[area].questions[i].options[res[formKey] - 1]
              ? this.testAdapter[area].questions[i].options[res[formKey] - 1].result_text
              :
                ""
          :
            ""
    ...
    areasAnswer.map((x: any)=>{
      ...
      if(typeof x.option_value === 'number'){
        this.areasValue += parseInt(x.option_value)
      }
    })
    ...
    this.requestServ.updateProjects({
      ...
      company: this.projectForm.value.nombre_startup,
